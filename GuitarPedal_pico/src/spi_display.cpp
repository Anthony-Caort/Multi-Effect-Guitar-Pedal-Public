...
screen_display::SPIDisplay::SPIDisplay(uint16_t w, uint16_t h, uint32_t baudrate, uint8_t dc_pin, SPI_Instance spi_inst) :
width(w), height(h), baudrate(baudrate), data_cmd_pin(dc_pin) {
    switch(spi_inst) {
        case SPI0: // changed the pins to match what i have because i dont want to rewire 3 small wires i refuse to
            spi_SCK_pin = 2; //PICO_DEFAULT_SPI_SCK_PIN;
            spi_TX_pin  = 3; //PICO_DEFAULT_SPI_TX_PIN;
            spi_CSN_pin = 5; //PICO_DEFAULT_SPI_CSN_PIN;
        break;
...
*/
void screen_display::SPIDisplay::begin(void) {
	hostInitialize();
	deviceInitialize();
}
...
void screen_display::SPIDisplay::end() {
	hostTerminate();
}
...
*/
void screen_display::SPIDisplay::hostInitialize() {
...
*/
void screen_display::SPIDisplay::hostTerminate() {
...
void screen_display::SPIDisplay::deviceInitialize() {	
}
...
**		updates the display.
*/
void screen_display::SPIDisplay::clear() {
	uint16_t w = 2 * width - 1; // reimplement 2 *  if doesnt work
	uint16_t h = height - 1;
...
uint8_t screen_display::SPIDisplay::writeSPI(uint8_t val) {
	// write to SPI
...
uint8_t screen_display::SPIDisplay::writeSPI(uint8_t val1, uint8_t val2) {
	// write to SPI
...
uint8_t screen_display::SPIDisplay::writeSPI(uint8_t *cmd, int nCmd, uint8_t *data, int nData) {
...
void screen_display::SPIDisplay::drawBitmap(uint16_t c1, uint16_t r1, uint16_t c2, uint16_t r2, uint8_t *pBmp)
{
